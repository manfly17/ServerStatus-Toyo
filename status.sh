...
Install_jq(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate "https://ghproxy.com/https://github.com/manfly17/jq/releases/download/jq-1.5/jq-linux64" -O ${jq_file}
		else
			wget --no-check-certificate "https://ghproxy.com/https://github.com/manfly17/jq/releases/download/jq-1.5/jq-linux32" -O ${jq_file}
		fi
